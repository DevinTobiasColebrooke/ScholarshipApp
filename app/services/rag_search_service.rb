...
class RagSearchService
  ...
  def initialize(query, search_provider_class:, transform: true)
    @query = query
  ...
  def search_and_synthesize
    ...
    Rails.logger.info "RAG: Using search provider: #{@search_provider_class.name}"

    # 2. Search Web
    search_results = @search_provider_class.search(search_query)
    ...
    [ context, source_urls.to_a, search_results ]
  end
