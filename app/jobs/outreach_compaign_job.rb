class OutreachCampaignJob < ApplicationJob
  ...
  def perform(profile_name)
    @profile_name = profile_name

    Rails.logger.info "Starting Outreach Campaign for Profile: #{@profile_name}"
    ...
    while next_org = find_next_organization_to_contact
      ...
      Rails.logger.info "Initiated outreach for: #{next_org.name} (ID: #{contact.id}). Next in queue."
    ...
    Rails.logger.info "Outreach Campaign for #{@profile_name} finished. All organizations processed."
  end
